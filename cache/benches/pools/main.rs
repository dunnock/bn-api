...
fn bench_sync_pool(c: &mut Criterion, mut pool: RedisR2D2, name: &str)
{
    ...
    group.bench_function(name, move |b| {
        b.iter_custom(|iters| {
            ...
            // benchmark body
            let res: Vec<Option<String>> = (1..iters).map(|_| {
				let start = std::time::Instant::now();
...
pub fn service_benches() {
    let mut criterion: ::criterion::Criterion<_> =
		::criterion::Criterion::default().configure_from_args().sample_size(10).noise_threshold(0.1);

	let mut rt = actix_rt::System::new("test");
		
	let mut sync_conn = RedisR2D2::create_connection_pool(URL, TIMEOUT, TIMEOUT, TIMEOUT);
	let async_conn = rt.block_on(RedisAsyncConnection::create_connection(URL, TIMEOUT, TIMEOUT)).expect("failed to create async pool");
	let redis_deadpool = RedisDeadpool::create_connection_pool(URL, TIMEOUT, TIMEOUT, TIMEOUT, None).expect("failed to create deadpool");
	let redis_bb8 = rt.block_on(RedisBB8::create_connection_pool(URL, TIMEOUT, TIMEOUT, TIMEOUT));
	let async_conn_pool = rt.block_on(RedisAsyncConnectionPool::create_connection_pool(URL, TIMEOUT, TIMEOUT, None)).expect("failed to create async pool");
	...
	for concurrency in &[1,4,8,16,32,64] {
		...
		bench_async_conn(&mut criterion, &mut rt, async_conn.clone(), "redis-async with single connection", *concurrency);
		bench_deadpool(&mut criterion, &mut rt, redis_deadpool.clone(), "deadpool-redis with pool of connections", *concurrency);
		bench_bb8(&mut criterion, &mut rt, redis_bb8.clone(), "bb8-redis with pool of connections", *concurrency);
		bench_async_conn_pool(&mut criterion, &mut rt, async_conn_pool.clone(), "redis-async-pool with pool of connections", *concurrency);
	}
}
